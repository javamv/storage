...
export class VideoService {
    ...
    private evaluateFrameQuality(metadata: any): any {
        ...
        if (metadata.width >= qualityCriteria.resolution['1080p'].width && metadata.height >= qualityCriteria.resolution['1080p'].height) {
            resolutionCategory = '1080p';
        } else if (metadata.width >= qualityCriteria.resolution['720p'].width && metadata.height >= qualityCriteria.resolution['720p'].height) {
        ...
        if (bitRatePerFrame >= qualityCriteria.bitRatePerFrame[resolutionCategory].high) {
            qualityScore += 3;
        } else if (bitRatePerFrame >= qualityCriteria.bitRatePerFrame[resolutionCategory].medium) {
            qualityScore += 2;
        } else if (bitRatePerFrame >= qualityCriteria.bitRatePerFrame[resolutionCategory].low) {
            qualityScore += 1;
        }
        ...
        const standardBitrate = qualityCriteria.resolution[resolutionCategory].standardBitrate;
    ...
    async cleanupDirectory(dir: string): Promise<void> {
        try {
            ...
            this.logger.error(`Error while cleaning up output directory: ${err.message}`);
        }
    }
}
